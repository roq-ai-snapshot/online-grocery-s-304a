...
import { useRoqClient, useGrocerFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<GrocerInterface, unknown>;

interface GrocerListPageProps {
...
export function GrocerListPage(props: GrocerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useGrocerFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.grocer.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: GrocerInterface) => {
    if (hasAccess('grocer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/grocers/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/grocers/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('grocer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/grocers/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('grocer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
