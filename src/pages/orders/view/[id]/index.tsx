...
function OrderViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; grocer: boolean } }>(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'grocer'],
          id,
        },
        'order',
      ),
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem
                  label="Delivery Date"
                  text={data?.delivery_date ? format(data?.delivery_date, 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('grocer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Grocer"
                    text={
                      <Link as={NextLink} href={`/grocers/view/${data?.grocer?.id}`}>
                        {data?.grocer?.name + ''}
                      </Link>
                    }
                  />
