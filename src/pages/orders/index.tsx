...
export function OrderListPage(props: OrderListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'grocer', 'order_item.count'],
  ...
  const columns: ColumnType[] = [
    ...
    },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.delivery_date ? format(record?.delivery_date, 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('grocer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'grocer',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/grocers/view/${record.grocer?.id}`}>
              {record.grocer?.name}
            </Link>
          ),
        }
