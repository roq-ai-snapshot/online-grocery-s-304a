...
import { useGrocerFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['store-manager', 'business-owner', 'inventory-specialist', 'customer-service-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useGrocerFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Grocers',
      path: isTenantUser ? `/grocers/view/${data?.id}` : '/grocers',
      entity: 'grocer',
